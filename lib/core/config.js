...
Config.prototype.loadLivepeerConfigFile = function() {
  // todo: spec out names for registration in the file itself for a dev chain
  var configObject = {
    "default": {
      "enabled": false,
      "bin": "livepeer",
      "datadir": `.embark/${this.env}/livepeer`,
      "chain": 'dev'
    }
  };

  let configFilePath = this._getFileOrOject(this.configDir, 'livepeer', 'livepeer');
  this.livepeerConfig = this._mergeConfig(configFilePath, configObject, this.env);
}
